...
function M.get_options(config, ngx)
	-- Build session options. Only set fields if configured, allowing Nginx vars to take precedence (or use defaults)
	local session_opts = {
		redis = {
	...
	-- Helper to merge two array tables
	...
	local options = {
		...
		filters = mergeArrays(config.filters, config.ignore_auth_filters),
