...
local function verify_bearer_jwt(oidcConfig)
	...
	local json, verify_err, _ = require("resty.openidc").bearer_jwt_verify(opts, claim_spec)
	if verify_err then
		kong.log.err("Bearer JWT verify failed: " .. verify_err)
		return nil
	end
...
local function make_oidc(oidcConfig)
	...
	-- Pass session_opts to authenticate for proper session configuration (SameSite=None, Redis, etc)
	local res, err =
		require("resty.openidc").authenticate(oidcConfig, ngx.var.request_uri, unauth_action, oidcConfig.session_opts)
...
function OidcHandler.access(_, config)
